<template>
  <label class="select cursor-pointer">
    <div class="imitate relative" :class="{ open: focus }">
      <span
        class="w-full flex ai-center jc-fs mob:jc-center white_space-nowrap pl-2 pr-6 pt-2 mob:py-0 pb-1 color-grey"
        @click="focusTrue()"
        >{{ value_name ? data[value_name] : data }}</span
      >
      <div
        class="options absolute flex col z-1 bg-white"
        v-if="focus"
        v-click-outside:focus
      >
        <div
          ...
          :key="value_name ? option[value_name] : option"
        >
